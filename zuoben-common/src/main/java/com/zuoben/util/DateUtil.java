...
public class DateUtil {
	...
	public static Date parseStrToDate(String time) {
		return parseStrToDate(time, "yyyy-MM-dd");
	}
	 ...
	 * 得到传入秒数之前或之后的时间
	 */
	public static Date getDateAfterSec(Long sec) {
	...
	/**
	 * 将date类型转换为java8新的日期类型localDate（非常方便处理时间）
	 *
	 * @param date
	 * @return
	 */
	...
	public static int daysBetween(LocalDate startDateInclusive, LocalDate endDateExclusive) {
		long va1 = startDateInclusive.toEpochDay();
		long va2 = endDateExclusive.toEpochDay();
		return (int) (va2 - va1);
	}
	/**
	 * 给指定日期指定时分秒后返回
	 ...
	 * @return
	 */
	public static Date setHourAndMinut(Date date,Integer h,Integer m){
		Calendar c = Calendar.getInstance();
		c.setTime(date);
